...
class ChangeRegionsDialog extends JDialog implements EcoSysObserver {
    ...
    private void initGUI() {
        ...
        JLabel helpText = new JLabel("<html>Select a region type, the rows/cols interval, and provide values for the parameters in the Value Column (default values are used for parameters without a specified value). </html>");
        helpText.setPreferredSize(new Dimension(750, 60));
        helpPanel.add(helpText, BorderLayout.CENTER);
        mainPanel.add(helpPanel);
        
        _dataTable = new JTable();
        DefaultTableModel model = new DefaultTableModel(new Object[][] {}, new String[] {"Key", "Value", "Description"}) {
        ...
        _regionsComboBox.addActionListener(e -> updateTableBasedOnRegionType((String)_regionsComboBox.getSelectedItem(), model));
    ...
    public void onRegister(double time, MapInfo map, List<AnimalInfo> animals) {
        // Refresh the dialog with initial data
    }

    public void onReset(double time, MapInfo map, List<AnimalInfo> animals) {
        // Refresh the dialog upon simulation reset
    }

    public void onAnimalAdded(double time, MapInfo map, List<AnimalInfo> animals, AnimalInfo a) {
        // Update dialog if needed when an animal is added
    }

    public void onRegionSet(int row, int col, MapInfo map, RegionInfo r) {
        // Update dialog if needed when a region is set
    }

    public void onAvanced(double time, MapInfo map, List<AnimalInfo> animals, double dt) {
        // Update dialog if needed when the simulation advances
    }
}
